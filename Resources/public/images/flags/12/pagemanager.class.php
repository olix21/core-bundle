...
class PageManager extends ClassManager
{
    ...
    public function extendMethod($object, $method)
    {
        ...
        } elseif ($method == 'show_translate') {
            $pageContentManager = new PageContentManager($this->db);
            $list = $pageContentManager->getList(['where' => 'idPage = '.$object->get('id')]);
            $return = '';
    ...
    public function getVersions($id)
    {
        ...
        if ($q['data'] == 0) {
            echo 'Aucune version antérieure détectée';
        } else {
